...
| How many midterm exams are scheduled for PHYS 130 Mechanics? | pass | fail | pass |
...
````text
There are three midterms scheduled for PHYS 130 Mechanics (source: `course_phys_130.txt` and `course_phys_130_exams.txt`).
````
...
This information came from `course_phys_130.txt` and `course_phys_130_exams.txt`.
````
...
````text
Based on the provided documents, students need a student account to access campus Wi-Fi (source: `admin_wifi_and_accounts.txt`).
````
